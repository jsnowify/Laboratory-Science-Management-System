...
export async function createApp() {
  const env = serverEnv();
  const app = Fastify({ logger: true, bodyLimit: 128 * 1024, routerOptions: { ignoreTrailingSlash: false } });
  await app.register(cors, { origin: env.FRONTEND_URL, credentials: true, methods: ["GET", "POST", "PATCH", "DELETE", "OPTIONS"] });
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    if (!request.url.startsWith("/api/v1/") || !["POST", "PATCH", "PUT", "DELETE"].includes(request.method)) return;
    if (!trustedMutationOrigin(request.headers, env.FRONTEND_URL)) {
      throw new AppError(403, "UNTRUSTED_ORIGIN", "This action must be started from the LSMS website.");
    }
  });
  ...
  app.route({
    ...
    handler: async (request, reply) => {
      ...
      if (request.method === "POST" && request.url.startsWith("/api/auth/change-password")) {
        strongPassword.parse((request.body as { newPassword?: unknown } | undefined)?.newPassword);
      }
      const passwordChange = request.method === "POST" && request.url.startsWith("/api/auth/change-password");
      const passwordSession = passwordChange ? await auth.api.getSession({ headers: fromNodeHeaders(request.headers) }) : null;
      ...
      if (passwordChange && response.ok && passwordSession) {
        try {
          const [actor] = await database().select({ id: users.id }).from(users).where(eq(users.authUserId, passwordSession.user.id)).limit(1);
      ...
      response.headers.forEach((value, key) => { if (key.toLowerCase() !== "set-cookie") reply.header(key, value); });
