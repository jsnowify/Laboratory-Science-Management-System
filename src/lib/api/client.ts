...
export function apiPath(path: string) {
  ...
  if (
    ...
    throw new Error("Use a versioned LSMS API path");
  return normalized;
}
...
export async function apiRequest<T>(
  ...
  const response = await fetch(url, {
    ...
    throw new ApiError(
      0,
      "NETWORK_ERROR",
      "Cannot connect to LSMS. Check your internet connection, then try again. Your changes have not been confirmed.",
    );
  });
  ...
  if (!response.ok) {
    ...
    const fallback =
      response.status === 401
        ...
        : response.status === 403
          ? "Your account cannot perform this action. Contact your laboratory administrator if you need access."
          : response.status === 409
            ? "This record changed or conflicts with an existing record. Refresh the page and review your details."
