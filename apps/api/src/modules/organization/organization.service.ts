...
const whereText = (
  q: string,
  code: typeof colleges.code,
  name: typeof colleges.name,
) => (q ? or(ilike(code, `%${q}%`), ilike(name, `%${q}%`)) : undefined);

export async function listColleges(input: ListInput) {
  const filter = and(
    input.includeInactive ? undefined : eq(colleges.isActive, true),
...
export async function updateCollege(
  id: string,
  input: z.infer<typeof organizationUpdate>,
  actorId: string,
) {
  return database().transaction(async (tx) => {
    const [row] = await tx
...
export async function listCourses(input: ListInput & { collegeId?: string }) {
  const filter = and(
    ...
    input.q
      ? or(
          ilike(courses.code, `%${input.q}%`),
          ilike(courses.name, `%${input.q}%`),
        )
      : undefined,
  );
  const [rows, totals] = await Promise.all([
...
async function requireActiveCollege(id: string) {
  const [college] = await database()
    ...
    .from(colleges)
    .where(and(eq(colleges.id, id), eq(colleges.isActive, true)))
    .limit(1);
  if (!college)
...
export async function updateCourse(
  id: string,
  input: z.infer<typeof courseUpdate>,
  actorId: string,
) {
...
export async function listDepartments(input: ListInput) {
  const filter = and(
    input.includeInactive ? undefined : eq(departments.isActive, true),
    input.q
      ? or(
          ilike(departments.code, `%${input.q}%`),
          ilike(departments.name, `%${input.q}%`),
        )
      : undefined,
  );
  const [rows, totals] = await Promise.all([
...
export async function updateDepartment(
  id: string,
  input: z.infer<typeof departmentUpdate>,
  actorId: string,
) {
  return database().transaction(async (tx) => {
    const [row] = await tx
