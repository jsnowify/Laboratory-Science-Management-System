...
export function RequestForm({ selectedEquipment, draft }: { selectedEquipment?: string; draft?: Draft }) {
  ...
  return <form onSubmit={save} className="workspace-card space-y-7 p-5 sm:p-8">
    <p className="border-b border-[#e5ece4] pb-6 text-sm leading-7 text-[#526b59]">Choose dates and equipment, then save a draft or submit for review.</p>
    <section className="space-y-4"><h3 className="flex items-center gap-2 text-lg text-[#254533]"><CalendarDays size={19} className="text-[#4e8260]" aria-hidden="true" /> Purpose and schedule</h3>
    ...
    <div className="grid gap-4 sm:grid-cols-2"><FormField id="borrowAt" label="Borrow date and time" error={fields.requestedBorrowAt?.[0]}><DateTimeField id="borrowAt" label="Borrow date and time" required value={borrowAt} onValueChange={setBorrowAt} /></FormField><FormField id="dueAt" label="Due date and time" error={fields.requestedDueAt?.[0]}><DateTimeField id="dueAt" label="Due date and time" min={borrowAt || undefined} required value={dueAt} onValueChange={setDueAt} /></FormField></div></section>
    <section className="border-t border-[#e5ece4] pt-6"><h3 className="mb-4 flex items-center gap-2 text-lg text-[#254533]"><Boxes size={19} className="text-[#4e8260]" aria-hidden="true" /> Equipment requested</h3><FormField id="catalogSearch" label="Search equipment types"><input id="catalogSearch" className={inputClass} placeholder="Filter equipment options" value={catalogSearch} onChange={(event) => setCatalogSearch(event.target.value)} /></FormField>
      {catalogError && <ErrorNotice message={catalogError} onRetry={() => setRetry((value) => value + 1)} />}<div className="mt-4 space-y-3">{items.map((item, index) => <div key={index} className="grid gap-3 rounded-xl border border-[#e1eae0] bg-[#f7faf6] p-3 sm:grid-cols-[1fr_100px_auto]"><CustomSelect id={`equipment-${index}`} label={`Equipment item ${index + 1}`} required value={item.equipmentCatalogId} placeholder="Choose equipment" emptyMessage="No equipment is available to request yet. Check back after the laboratory adds equipment." options={[...catalog.map((option) => ({ value: option.id, label: `${option.equipmentName} (${option.availableUnits} available now)` })), ...(item.equipmentCatalogId && !catalog.some((option) => option.id === item.equipmentCatalogId) ? [{ value: item.equipmentCatalogId, label: "Selected equipment" }] : [])]} onValueChange={(value) => setItems(items.map((row, position) => position === index ? { ...row, equipmentCatalogId: value } : row))} /><input aria-label={`Quantity for item ${index + 1}`} type="number" min={1} max={100} required className={inputClass} value={item.quantityRequested} onChange={(event) => setItems(items.map((row, position) => position === index ? { ...row, quantityRequested: Number(event.target.value) } : row))} /><button type="button" disabled={items.length === 1} onClick={() => setItems(items.filter((_, position) => position !== index))} className="ui-button-secondary">Remove</button></div>)}</div>
    ...
    <div className="flex flex-wrap gap-3 border-t border-[#e5ece4] pt-6"><button type="submit" value="draft" disabled={busy} className="ui-button-secondary">{busy ? "Saving…" : "Save draft"}</button><button type="submit" value="submit" disabled={busy} className="ui-button-primary">Submit request</button><Link href="/portal/requests/" className="ui-button-secondary">Back to requests</Link></div>
  </form>;
}
