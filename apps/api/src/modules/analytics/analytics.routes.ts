...
export async function analyticsRoutes(app: FastifyInstance) {
  app.get("/api/v1/analytics/:key/", async (request) => {
    ...
    const key = z.enum(Object.keys(analyticsViews) as [keyof typeof analyticsViews, ...(keyof typeof analyticsViews)[]]).parse((request.params as { key: string }).key);
    const { page, limit } = listQuery.parse(request.query);
    const view = sql.raw(analyticsViews[key]);
    const [data, totals] = await Promise.all([
      database().execute(sql`select * from ${view} limit ${limit} offset ${(page - 1) * limit}`),
  ...
  app.get("/api/v1/reports/:key/", async (request) => {
    ...
    const key = z.enum(Object.keys(reports) as [keyof typeof reports, ...(keyof typeof reports)[]]).parse((request.params as { key: string }).key);
    if (!(reports[key].roles as readonly string[]).includes(actor.role)) throw new AppError(403, "FORBIDDEN", "This report is not available to your role.");
    const { page, limit } = listQuery.parse(request.query);
    const source = sql.raw(reports[key].source);
    const [data, totals] = await Promise.all([
      database().execute(sql`select * from ${source} limit ${limit} offset ${(page - 1) * limit}`),
  ...
  app.get("/api/v1/reports/:key/csv/", async (request, reply) => {
    ...
    const key = z.enum(Object.keys(reports) as [keyof typeof reports, ...(keyof typeof reports)[]]).parse((request.params as { key: string }).key);
    if (!(reports[key].roles as readonly string[]).includes(actor.role)) throw new AppError(403, "FORBIDDEN", "This report is not available to your role.");
    const rows = await database().execute(sql`select * from ${sql.raw(reports[key].source)} limit 10000`);
