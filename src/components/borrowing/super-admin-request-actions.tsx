...
export function SuperAdminRequestActions({ request }: { request: RequestDetail }) {
  ...
  return <section className="ui-panel max-w-4xl space-y-5 p-4 sm:p-6" aria-busy={busy}>
    ...
    {canAllocate && <div className="space-y-4"><div><h2 className="text-lg font-semibold">Allocate physical assets</h2><p className="mt-1 text-sm text-slate-600">{request.allocations.length} of {request.items.reduce((sum, item) => sum + (item.quantityApproved ?? 0), 0)} approved units allocated. Select the equipment, then assign an available asset.</p></div>
      <div className="grid gap-4 sm:grid-cols-2"><FormField id="approved-item" label="Approved equipment"><select id="approved-item" className={inputClass} value={itemId} onChange={(event) => { setItemId(event.target.value); setAssetId(""); setAssets([]); setAssetLoading(Boolean(event.target.value)); setAssetError(""); }}><option value="">Choose equipment</option>{remainingItems.map((item) => <option key={item.id} value={item.id}>{item.equipmentName}</option>)}</select></FormField><FormField id="physical-asset" label="Physical asset"><select id="physical-asset" disabled={!itemId || assetLoading} className={inputClass} value={assetId} onChange={(event) => setAssetId(event.target.value)}><option value="">{assetLoading ? "Loading assets..." : "Choose available asset"}</option>{availableAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.assetCode}</option>)}</select></FormField></div>
    ...
    {canRelease && <div className="space-y-4"><div><h2 className="text-lg font-semibold">Inspect and release equipment</h2><p className="mt-1 text-sm text-slate-600">All approved units are allocated. Inspect each asset and record its condition before handover.</p></div><div className="grid gap-4 sm:grid-cols-2">{request.allocations.map((allocation) => <FormField key={allocation.id} id={`condition-${allocation.id}`} label={`${allocation.assetCode} condition`}><select id={`condition-${allocation.id}`} className={inputClass} value={conditions[allocation.id] ?? ""} onChange={(event) => setConditions({ ...conditions, [allocation.id]: event.target.value as Condition })}><option value="">Choose condition</option>{["excellent", "good", "fair", "damaged"].map((value) => <option key={value} value={value}>{value[0].toUpperCase() + value.slice(1)}</option>)}</select></FormField>)}</div><p className="text-xs text-slate-600">A condition is required for every asset.</p><button type="button" disabled={busy || request.allocations.length === 0 || request.allocations.some((allocation) => !conditions[allocation.id])} onClick={() => action("release", { allocations: request.allocations.map((allocation) => ({ allocationId: allocation.id, condition: conditions[allocation.id] })) })} className="ui-button-primary">{busy ? "Releasing..." : "Confirm physical release"}</button></div>}
