...
function validateSchedule(input: RequestInput) {
  if (Date.parse(input.requestedBorrowAt) < Date.now() - 60_000)
    throw new AppError(
      422,
      "INVALID_SCHEDULE",
...
export async function createDraft(input: RequestInput, borrowerId: string) {
  ...
  return database().transaction(async (tx) => {
    ...
    const activeCatalog = await tx
      ...
      .where(
        and(
          inArray(equipmentCatalog.id, ids),
...
export async function listRequests(
  ...
  const filter = and(
    actor.role === "student_faculty"
      ? eq(borrowRequests.borrowerId, actor.id)
      : undefined,
    input.history
      ? inArray(borrowRequests.status, ["rejected", "returned", "cancelled"])
      : undefined,
    input.status ? eq(borrowRequests.status, input.status) : undefined,
    input.q
      ? or(
          ilike(borrowRequests.requestNumber, `%${input.q}%`),
          ilike(borrowRequests.purpose, `%${input.q}%`),
        )
      : undefined,
  );
...
export async function getRequest(
  ...
  const [items, history] = await Promise.all([
    database()
      ...
      .innerJoin(
        equipmentCatalog,
        eq(borrowRequestItems.equipmentCatalogId, equipmentCatalog.id),
      )
      .where(eq(borrowRequestItems.borrowRequestId, id)),
    database()
      .select({
      ...
      .where(eq(borrowRequestStatusHistory.borrowRequestId, id))
...
export async function submitRequest(id: string, borrowerId: string) {
  return database().transaction(async (tx) => {
    ...
    if (Date.parse(request.requestedBorrowAt) < Date.now() - 60_000)
      throw new AppError(
        422,
        "INVALID_SCHEDULE",
        "Borrow date must be in the future.",
  ...
  return database().transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      await tx
        ...
        .where(eq(borrowRequestItems.id, item.id));
    }
    const [updated] = await tx
      .update(borrowRequests)
      .set({
...
export async function rejectRequest(
  id: string,
  actorId: string,
  input: z.infer<typeof rejectionInput>,
) {
  return database().transaction(async (tx) => {
    const [request] = await tx
      .select()
