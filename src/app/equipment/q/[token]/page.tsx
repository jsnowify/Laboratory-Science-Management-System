...
export default async function QrLookupPage({
  ...
  return (
    <DashboardShell profile={profile} title={`Equipment ${asset.assetCode}`}>
      <dl className="grid max-w-2xl gap-4 rounded-xl border border-slate-200 bg-white p-6 sm:grid-cols-2">
        {[
          ...
          ["Department", asset.departmentName ?? "Unassigned"],
          ["Condition", asset.currentCondition],
          ["Operational status", asset.operationalStatus],
          ["Current availability", asset.availabilityStatus],
        ].map(([label, value]) => (
          <div key={label}>
