...
export async function dashboardRoutes(app: FastifyInstance) {
  app.get("/api/v1/dashboard/", async (request) => {
    ...
    if (actor.role === "super_admin") {
      const [requests, custody, overdue, returns, audit, activity] = await Promise.all([
        ...
        database().execute(sql`select count(*)::int as total, count(*) filter (where u.role = 'admin')::int as admin_total from audit_logs a left join users u on u.id = a.actor_user_id`),
        database().execute(sql`select a.id, a.action, a.entity_type as "entityType", a.created_at as "createdAt", u.first_name as "firstName", u.last_name as "lastName", u.role as "actorRole" from audit_logs a left join users u on u.id = a.actor_user_id order by a.created_at desc, a.id desc limit 8`),
      ]);
