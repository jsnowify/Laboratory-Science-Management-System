...
import { z } from "zod";
import { courseInput, courseUpdate, departmentInput, departmentUpdate, organizationInput, organizationListQuery, organizationUpdate, uuidParam } from "@lsms/shared";
import { requirePermission } from "../../auth/authorize";
import { createCollege, createCourse, createDepartment, listColleges, listCourses, listDepartments, updateCollege, updateCourse, updateDepartment } from "./organization.service";
...
export async function organizationRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/courses/", async (request) => {
    const input = organizationListQuery.extend({ collegeId: z.uuid().optional() }).parse(request.query);
    if (input.includeInactive) await requirePermission(request, "organization.read");
