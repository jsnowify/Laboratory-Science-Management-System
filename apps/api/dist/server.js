...
var registrationInput = identityInput
  .extend({
  ...
  .refine(
    (value) => value.personType !== "student" || Boolean(value.courseId),
    {
      path: ["courseId"],
...
var departmentInput = organizationInput.extend({
...
var organizationUpdate = organizationInput
  .partial()
...
var departmentUpdate = departmentInput
  .partial()
  .extend({ isActive: z.boolean().optional() })
  .refine(
    (value) => Object.keys(value).length > 0,
...
var categoryUpdate = categoryInput
  .partial()
  .extend({ isActive: z.boolean().optional() })
...
var catalogUpdate = catalogInput
  .partial()
  .extend({ isActive: z.boolean().optional() })
  .refine((value) => Object.keys(value).length > 0);
...
var assetUpdate = assetInput
  .partial()
  .extend({ archive: z.boolean().optional() })
  .refine((value) => Object.keys(value).length > 0);
...
var borrowRequestInput = z
  .object({
    ...
    items: z
      .array(
        z.object({
          equipmentCatalogId: z.uuid(),
          quantityRequested: z.number().int().min(1).max(100),
        }),
      )
      .min(1)
      .max(20),
  ...
  .refine(
    (value) =>
      Date.parse(value.requestedDueAt) > Date.parse(value.requestedBorrowAt),
    {
      path: ["requestedDueAt"],
  ...
  .refine(
    (value) =>
      new Set(value.items.map((item) => item.equipmentCatalogId)).size ===
      value.items.length,
    { path: ["items"], message: "Each equipment type can appear once." },
  );
var rejectionInput = z.object({
  reason: z.string().trim().min(5).max(2e3),
...
var approvalInput = z
  ...
  .refine((value) => value.items.some((item) => item.quantityApproved > 0), {
    path: ["items"],
    message: "Approve at least one unit.",
  });
...
var releaseInput = z
  ...
  .refine(
    (value) =>
      new Set(value.allocations.map((item) => item.allocationId)).size ===
      value.allocations.length,
    { path: ["allocations"], message: "Each allocation can appear once." },
  );
var returnInput = z
  .object({
    allocationId: z.uuid(),
  ...
  .refine((value) => value.outcome === "normal" || Boolean(value.remarks), {
    path: ["remarks"],
    message: "Add remarks for damage or maintenance.",
  })
  .refine(
    (value) =>
      value.conditionAfter !== "damaged" || value.outcome === "damaged",
    {
      path: ["outcome"],
      message: "A damaged condition requires a damaged outcome.",
    },
  );
var borrowListQuery = listQuery.extend({
  status: z
    .enum([
      "draft",
      "submitted",
...
var accountStatus = pgEnum("account_status", [
  "pending",
  "active",
  "suspended",
  "archived",
]);
var allocationStatus = pgEnum("allocation_status", [
  "reserved",
  "released",
  "returned",
  "cancelled",
]);
var borrowRequestStatus = pgEnum("borrow_request_status", [
  "draft",
  "submitted",
  "under_review",
  "approved",
...
var equipmentCondition = pgEnum("equipment_condition", [
  "excellent",
  "good",
  "fair",
  "damaged",
]);
var equipmentOperationalStatus = pgEnum("equipment_operational_status", [
  "active",
  "maintenance",
  "damaged",
  "retired",
...
var returnOutcome = pgEnum("return_outcome", [
  "normal",
  "damaged",
  "maintenance_required",
]);
var userRole = pgEnum("user_role", ["super_admin", "admin", "student_faculty"]);
var borrowRequestNumberSeq = pgSequence("borrow_request_number_seq", {
  startWith: "1",
  increment: "1",
  minValue: "1",
...
var isoRequisitionNumberSeq = pgSequence("iso_requisition_number_seq", {
  startWith: "1",
  increment: "1",
  minValue: "1",
  maxValue: "9223372036854775807",
...
var equipmentCategories = pgTable(
...
var borrowRequestItems = pgTable(
...
var borrowRequestStatusHistory = pgTable(
...
var authUser = pgTable2("auth_users", {
  ...
  image: text2("image"),
  createdAt: timestamp2("created_at", { withTimezone: true })
    .notNull()
    .defaultNow(),
  updatedAt: timestamp2("updated_at", { withTimezone: true })
    .notNull()
    .defaultNow(),
});
...
function database() {
  ...
  return drizzle(client, {
    schema: { ...schema_exports, ...auth_schema_exports },
  });
}
...
function canSuperAdminChangeStatus(
  targetRole,
  from,
  to,
  isSelf,
...
async function listManageableUsers(input, actorRole) {
  const filter = and(
    actorRole === "admin"
      ? eq2(users.role, "student_faculty")
      : input.role
        ? eq2(users.role, input.role)
        : void 0,
...
async function changeUserStatus(id, input, actorId, actorRole) {
  return database().transaction(async (tx) => {
    if (actorRole === "super_admin")
      await tx.execute(sql2`select pg_advisory_xact_lock(742611931)`);
    ...
    if (
      ...
      const [other] = await tx
        ...
        .where(
          and(
            eq2(users.role, "super_admin"),
            eq2(users.accountStatus, "active"),
            ne(users.id, id),
          ),
        )
        .limit(1);
...
async function updateAccountDetails(id, input, actorId, actorRole) {
  ...
  return database().transaction(async (tx) => {
    ...
    const [duplicate] = await tx
      .select({ id: users.id })
      .from(users)
      .where(eq2(users.email, input.email))
      .limit(1);
    ...
    const [authRow] = await tx
      ...
      .set({
        ...
        email: input.email,
        emailVerified:
          target.email.toLowerCase() === input.email ? void 0 : false,
        updatedAt: /* @__PURE__ */ new Date(),
      })
      .where(eq2(authUser.id, target.authUserId))
      ...
      .values({
        ...
        metadata: {
          ...
          changedFields: [
            ...
            (field) =>
              String(target[field] ?? "") !== String(input[field] ?? ""),
          ),
        },
      });
    return row;
  ...
  try {
    return await database().transaction(async (tx) => {
      ...
      const [existing] = await tx
        ...
        .where(
          and2(
            eq3(users.role, "super_admin"),
            eq3(users.accountStatus, "active"),
          ),
        )
        .limit(1);
      if (existing)
  ...
  try {
    return await database().transaction(async (tx) => {
      const [existingProfile] = await tx
        ...
        .where(
          or2(
            eq3(users.email, input.email),
            eq3(users.institutionalId, input.institutionalId),
          ),
        )
        .limit(1);
...
var whereText = (q, code, name2) =>
  q ? or3(ilike2(code, `%${q}%`), ilike2(name2, `%${q}%`)) : void 0;
async function listColleges(input) {
  const filter = and3(
    input.includeInactive ? void 0 : eq4(colleges.isActive, true),
    whereText(input.q, colleges.code, colleges.name),
  );
  const [rows, totals] = await Promise.all([
    database()
      .select()
...
async function updateCollege(id, input, actorId) {
  return database().transaction(async (tx) => {
    ...
    await tx
      ...
      .values({
        actorUserId: actorId,
        action: row.isActive ? "college.updated" : "college.deactivated",
        entityType: "colleges",
        entityId: id,
      });
    return row;
...
async function listCourses(input) {
  const filter = and3(
    ...
    input.q
      ? or3(
          ilike2(courses.code, `%${input.q}%`),
          ilike2(courses.name, `%${input.q}%`),
        )
      : void 0,
  );
  const [rows, totals] = await Promise.all([
...
async function requireActiveCollege(id) {
  const [college] = await database()
    .select({ id: colleges.id })
    .from(colleges)
    .where(and3(eq4(colleges.id, id), eq4(colleges.isActive, true)))
    .limit(1);
  if (!college)
    throw new AppError(422, "INVALID_COLLEGE", "Choose an active college.");
}
...
async function updateCourse(id, input, actorId) {
  ...
  return database().transaction(async (tx) => {
    ...
    await tx
      ...
      .values({
        actorUserId: actorId,
        action: row.isActive ? "course.updated" : "course.deactivated",
        entityType: "courses",
        entityId: id,
      });
    return row;
...
async function listDepartments(input) {
  const filter = and3(
    ...
    input.q
      ? or3(
          ilike2(departments.code, `%${input.q}%`),
          ilike2(departments.name, `%${input.q}%`),
        )
      : void 0,
  );
  const [rows, totals] = await Promise.all([
...
async function updateDepartment(id, input, actorId) {
  return database().transaction(async (tx) => {
    ...
    await tx
      ...
      .values({
        actorUserId: actorId,
        action: row.isActive ? "department.updated" : "department.deactivated",
        entityType: "departments",
        entityId: id,
      });
    return row;
...
async function organizationRoutes(app2) {
  ...
  app2.get("/api/v1/courses/", async (request) => {
    const input = organizationListQuery
      .extend({ collegeId: z3.uuid().optional() })
      .parse(request.query);
    if (input.includeInactive)
      await requirePermission(request, "organization.read");
...
async function accountsRoutes(app2) {
  app2.get("/api/v1/users/", async (request) => {
    ...
    if (
      ...
      !can(actor.role, "users.manage_all")
    )
      throw new AppError(403, "FORBIDDEN", "You cannot view user accounts.");
...
async function updateCategory(id, input, actorId) {
  return database().transaction(async (tx) => {
    const [row] = await tx
      ...
      .set(input)
      .where(eq5(equipmentCategories.id, id))
      .returning();
...
async function listCatalog(query) {
  const filter = and4(
    ...
    query.categoryId
      ? eq5(equipmentCatalog.categoryId, query.categoryId)
      : void 0,
  ...
  const [data, totals] = await Promise.all([
    database()
      ...
      .leftJoin(
        ...
        eq5(
          equipmentCatalog.id,
          vwEquipmentInventorySummary.equipmentCatalogId,
        ),
      )
      .where(filter)
...
async function requireActiveCategory(id) {
  const [row] = await database()
    ...
    .where(
      and4(
        eq5(equipmentCategories.id, id),
        eq5(equipmentCategories.isActive, true),
      ),
    )
    .limit(1);
  if (!row)
...
async function updateCatalog(id, input, actorId) {
  ...
  return database().transaction(async (tx) => {
    const [row] = await tx
      ...
      .where(eq5(equipmentCatalog.id, id))
      .returning();
...
async function listAssets(query) {
  const filter = and4(
    ...
    query.catalogId
      ? eq5(equipmentAssets.equipmentCatalogId, query.catalogId)
      : void 0,
    query.q
      ? or4(
          ilike3(equipmentAssets.assetCode, `%${query.q}%`),
          ilike3(equipmentAssets.serialNumber, `%${query.q}%`),
        )
      : void 0,
  );
  const [data, totals] = await Promise.all([
    database()
      ...
      .leftJoin(
        ...
        eq5(equipmentAssets.id, vwEquipmentAssetAvailability.equipmentAssetId),
      )
      .where(filter)
...
async function requireValidAssetRelations(input) {
  if (input.equipmentCatalogId) {
    const [row] = await database()
      ...
      .where(
        and4(
          eq5(equipmentCatalog.id, input.equipmentCatalogId),
          eq5(equipmentCatalog.isActive, true),
        ),
      )
      .limit(1);
    if (!row)
  ...
  if (input.departmentId) {
    const [row] = await database()
      ...
      .where(
        and4(
          eq5(departments.id, input.departmentId),
          eq5(departments.isActive, true),
        ),
      )
      .limit(1);
    if (!row)
  ...
  return database().transaction(async (tx) => {
    const [row] = await tx
      ...
      .set({
        ...changes,
        archivedAt:
          archive === true ? /* @__PURE__ */ new Date().toISOString() : void 0,
      })
      .where(eq5(equipmentAssets.id, id))
      .returning();
    if (!row)
...
async function lookupQr(token) {
  const [row] = await database()
    ...
    .where(eq5(vwEquipmentQrLookup.qrToken, token))
    .limit(1);
  if (!row)
...
async function equipmentRoutes(app2) {
  app2.get("/api/v1/equipment/categories/", async (request) => {
    ...
    if (query.includeInactive)
      await requirePermission(request, "equipment.write");
  ...
  app2.post("/api/v1/equipment/categories/", async (request, reply) => {
    ...
    return reply
      .code(201)
      .send(await createCategory(categoryInput.parse(request.body), actor.id));
  });
  app2.patch("/api/v1/equipment/categories/:id/", async (request) => {
    const actor = await requirePermission(request, "equipment.write");
    return updateCategory(
      uuidParam.parse(request.params).id,
      categoryUpdate.parse(request.body),
      actor.id,
    );
  });
  ...
  app2.post("/api/v1/equipment/catalog/", async (request, reply) => {
    ...
    return reply
      .code(201)
      .send(await createCatalog(catalogInput.parse(request.body), actor.id));
  });
  app2.patch("/api/v1/equipment/catalog/:id/", async (request) => {
    const actor = await requirePermission(request, "equipment.write");
    return updateCatalog(
      uuidParam.parse(request.params).id,
      catalogUpdate.parse(request.body),
      actor.id,
    );
  });
  app2.get("/api/v1/equipment/assets/", async (request) => {
    const actor = await requireActiveProfile(request);
    if (actor.role === "student_faculty")
      throw new AppError(
        403,
        "FORBIDDEN",
        "Asset management is restricted to staff.",
      );
    const query = equipmentListQuery.parse(request.query);
    if (query.includeInactive && actor.role !== "admin")
      throw new AppError(
        403,
        "FORBIDDEN",
        "Archived inventory is restricted to Admin.",
      );
  ...
  app2.post("/api/v1/equipment/assets/", async (request, reply) => {
    ...
    return reply
      .code(201)
      .send(await createAsset(assetInput.parse(request.body), actor.id));
  });
  app2.patch("/api/v1/equipment/assets/:id/", async (request) => {
    const actor = await requirePermission(request, "equipment.write");
    return updateAsset(
      uuidParam.parse(request.params).id,
      assetUpdate.parse(request.body),
      actor.id,
    );
  });
  app2.get("/api/v1/equipment/assets/:id/qr/", async (request, reply) => {
    ...
    const [asset] = await database()
      .select({ qrToken: equipmentAssets.qrToken })
      .from(equipmentAssets)
      .where(eq6(equipmentAssets.id, id))
      .limit(1);
    if (!asset)
      throw new AppError(404, "NOT_FOUND", "Equipment asset not found.");
...
async function allocateAsset(requestId, actorId, input) {
  return database().transaction(async (tx) => {
    const [request] = await tx
      .select()
      .from(borrowRequests)
      .where(eq7(borrowRequests.id, requestId))
      .for("update")
      .limit(1);
    ...
    if (request.status !== "approved" && request.status !== "ready_for_release")
      throw new AppError(
        409,
        "INVALID_STATUS",
        "Approve the request before allocating assets.",
      );
    const [item] = await tx
      .select()
      .from(borrowRequestItems)
      .where(eq7(borrowRequestItems.id, input.itemId))
      .for("update")
      .limit(1);
    if (!item || item.borrowRequestId !== requestId || !item.quantityApproved)
      throw new AppError(
        422,
        "INVALID_ITEM",
        "Choose an approved request item.",
      );
    const [asset] = await tx
      .select()
      .from(equipmentAssets)
      .where(eq7(equipmentAssets.id, input.assetId))
      .limit(1);
    if (
      !asset ||
      asset.equipmentCatalogId !== item.equipmentCatalogId ||
      ...
      throw new AppError(
        422,
        "INVALID_ASSET",
        "Choose an active, compatible physical asset.",
      );
    const [overlap] = await tx
      .select({ id: borrowAllocations.id })
    ...
    const [allocated] = await tx
      ...
      .where(
        and5(
          eq7(borrowAllocations.borrowRequestItemId, item.id),
          inArray(borrowAllocations.allocationStatus, ["reserved", "released"]),
        ),
      );
    if (allocated.total >= item.quantityApproved)
      throw new AppError(
        409,
        "QUANTITY_FILLED",
        "This item already has its approved number of assets.",
      );
    const [row] = await tx
      .insert(borrowAllocations)
      .values({
    ...
    const items = await tx
      ...
      .from(borrowRequestItems)
      .where(eq7(borrowRequestItems.borrowRequestId, requestId));
    const [total] = await tx
      .select({ value: count4() })
      .from(borrowAllocations)
      .innerJoin(
        borrowRequestItems,
        eq7(borrowAllocations.borrowRequestItemId, borrowRequestItems.id),
      )
      .where(
        and5(
          eq7(borrowRequestItems.borrowRequestId, requestId),
          inArray(borrowAllocations.allocationStatus, ["reserved", "released"]),
        ),
      );
    if (
      total.value ===
        items.reduce(
          (sum, current) => sum + (current.quantityApproved ?? 0),
          0,
        ) &&
  ...
  return database().transaction(async (tx) => {
    ...
    if (request.status !== "ready_for_release")
      throw new AppError(
        409,
        "INVALID_STATUS",
        "All approved assets must be allocated before release.",
      );
    ...
    if (
      ...
      allocations.some(
        (row) =>
          row.allocationStatus !== "reserved" ||
          !input.allocations.some((item) => item.allocationId === row.id),
      )
    )
      throw new AppError(
        422,
        "INVALID_ALLOCATIONS",
        "Release every reserved asset in this request together.",
      );
    const releasedAt = /* @__PURE__ */ new Date().toISOString();
    for (const item of input.allocations) {
      await tx
        .update(borrowAllocations)
        .set({
...
async function processReturn(actorId, input) {
  return database().transaction(async (tx) => {
    const [allocation] = await tx
      ...
      .where(eq7(borrowAllocations.id, input.allocationId))
      .for("update")
      .limit(1);
    if (!allocation)
      throw new AppError(404, "NOT_FOUND", "Custody record not found.");
    if (allocation.allocationStatus !== "released")
      throw new AppError(
        409,
        "INVALID_STATUS",
    ...
    const [request] = await tx
      .select()
      .from(borrowRequests)
      .where(eq7(borrowRequests.id, allocation.borrowRequestId))
      .for("update")
      .limit(1);
    if (
      !request ||
      ...
      throw new AppError(
        409,
        "INVALID_STATUS",
        "This request is not in active custody.",
      );
    ...
    const [totals] = await tx
      .select({
        total: count4(),
        returned: sql4`count(*) filter (where ${borrowAllocations.allocationStatus} = 'returned')::int`,
      })
      .from(borrowAllocations)
      .innerJoin(
        borrowRequestItems,
      ...
      .where(
        and5(
          eq7(borrowRequestItems.borrowRequestId, request.id),
          inArray(borrowAllocations.allocationStatus, ["released", "returned"]),
        ),
      );
    ...
    await tx
      .update(borrowRequests)
      .set({ status })
      .where(eq7(borrowRequests.id, request.id));
    await tx.insert(auditLogs).values({
      actorUserId: actorId,
...
async function listCustody(page, limit, borrowerId) {
  const filter = borrowerId
    ? eq7(vwCurrentCustody.borrowerId, borrowerId)
    : void 0;
  const [data, totals] = await Promise.all([
    database()
      .select()
      .from(vwCurrentCustody)
      .where(filter)
      .orderBy(vwCurrentCustody.dueAt)
      .limit(limit)
      .offset((page - 1) * limit),
...
async function listAccountability(page, limit, borrowerId) {
  const filter = borrowerId
    ? eq7(vwAccountabilityRecords.borrowerId, borrowerId)
    : void 0;
  const [data, totals] = await Promise.all([
    database()
      .select()
      .from(vwAccountabilityRecords)
      .where(filter)
      .orderBy(vwAccountabilityRecords.releasedAt)
      .limit(limit)
      .offset((page - 1) * limit),
    database()
      .select({ total: count4() })
...
function validateSchedule(input) {
  if (Date.parse(input.requestedBorrowAt) < Date.now() - 6e4)
    throw new AppError(
      422,
      "INVALID_SCHEDULE",
      "Borrow date must be in the future.",
    );
}
async function createDraft(input, borrowerId) {
  ...
  return database().transaction(async (tx) => {
    ...
    const activeCatalog = await tx
      .select({ id: equipmentCatalog.id })
      .from(equipmentCatalog)
      .where(
        and6(
          inArray2(equipmentCatalog.id, ids),
          eq8(equipmentCatalog.isActive, true),
        ),
      );
    if (activeCatalog.length !== ids.length)
      throw new AppError(
        422,
        "INVALID_EQUIPMENT",
        "Choose active equipment types.",
      );
    const [request] = await tx
    ...
    await tx
      ...
      .values(
        input.items.map((item) => ({ ...item, borrowRequestId: request.id })),
      );
    await tx
      .insert(auditLogs)
      .values({
        actorUserId: borrowerId,
        action: "borrow_request.draft_created",
        entityType: "borrow_requests",
        entityId: request.id,
      });
    return request;
...
async function updateDraft(id, input, borrowerId) {
  ...
  return database().transaction(async (tx) => {
    const [request] = await tx
      .select()
      .from(borrowRequests)
      .where(eq8(borrowRequests.id, id))
      .for("update")
      .limit(1);
    if (!request || request.borrowerId !== borrowerId)
    ...
    await tx
      .delete(borrowRequestItems)
      .where(eq8(borrowRequestItems.borrowRequestId, id));
    await tx
      .insert(borrowRequestItems)
      .values(input.items.map((item) => ({ ...item, borrowRequestId: id })));
    const [updated] = await tx
      .update(borrowRequests)
      .set({
        purpose: input.purpose,
    ...
    await tx
      ...
      .values({
        actorUserId: borrowerId,
        action: "borrow_request.draft_updated",
        entityType: "borrow_requests",
        entityId: id,
      });
    return updated;
...
async function listRequests(input, actor) {
  if (actor.role === "admin")
    throw new AppError(
      403,
      "FORBIDDEN",
      "Borrowing operations are restricted to the Super Admin.",
    );
  const filter = and6(
    actor.role === "student_faculty"
      ? eq8(borrowRequests.borrowerId, actor.id)
      : void 0,
    input.history
      ? inArray2(borrowRequests.status, ["rejected", "returned", "cancelled"])
      : void 0,
    input.status ? eq8(borrowRequests.status, input.status) : void 0,
    input.q
      ? or5(
          ilike4(borrowRequests.requestNumber, `%${input.q}%`),
          ilike4(borrowRequests.purpose, `%${input.q}%`),
        )
      : void 0,
  );
  const [data, totals] = await Promise.all([
    database()
      ...
      .from(borrowRequests)
      .innerJoin(users, eq8(borrowRequests.borrowerId, users.id))
      .where(filter)
...
async function getRequest(id, actor) {
  ...
  if (
    !request ||
    (actor.role === "student_faculty" && request.borrowerId !== actor.id)
  )
    throw new AppError(404, "NOT_FOUND", "Request not found.");
  const [items, history] = await Promise.all([
    database()
      ...
      .innerJoin(
        equipmentCatalog,
        eq8(borrowRequestItems.equipmentCatalogId, equipmentCatalog.id),
      )
      .where(eq8(borrowRequestItems.borrowRequestId, id)),
    database()
      .select({
      ...
      .from(borrowRequestStatusHistory)
      .where(eq8(borrowRequestStatusHistory.borrowRequestId, id))
      .orderBy(desc(borrowRequestStatusHistory.changedAt)),
  ]);
  const allocations =
    actor.role === "super_admin"
      ? await database()
          ...
          .innerJoin(
            equipmentAssets,
            eq8(borrowAllocations.equipmentAssetId, equipmentAssets.id),
          )
          .where(
            inArray2(
...
async function submitRequest(id, borrowerId) {
  return database().transaction(async (tx) => {
    ...
    if (Date.parse(request.requestedBorrowAt) < Date.now() - 6e4)
      throw new AppError(
        422,
        "INVALID_SCHEDULE",
        "Borrow date must be in the future.",
    ...
    if (itemCount.value < 1)
      throw new AppError(
        422,
        "EMPTY_REQUEST",
        "Add equipment before submitting.",
      );
    const [updated] = await tx
      .update(borrowRequests)
      .set({
    ...
    await tx
      ...
      .values({
        actorUserId: borrowerId,
        action: "borrow_request.submitted",
        entityType: "borrow_requests",
        entityId: id,
      });
    await tx.insert(notifications).values({
  ...
  return database().transaction(async (tx) => {
    ...
    if (request.status !== "submitted")
      throw new AppError(
        409,
        "INVALID_STATUS",
        "Only submitted requests can move under review.",
      );
    const [updated] = await tx
      .update(borrowRequests)
      .set({
    ...
    await tx
      ...
      .values({
        actorUserId: actorId,
        action: "borrow_request.review_started",
        entityType: "borrow_requests",
        entityId: id,
      });
    return updated;
...
async function approveRequest(id, actorId, input) {
  return database().transaction(async (tx) => {
    ...
    if (
      ...
      new Set(input.items.map((item) => item.itemId)).size !== items.length
    )
      throw new AppError(
        422,
        "INVALID_ITEMS",
        "Provide an approved quantity for every requested item.",
      );
    for (const item of items) {
      const approval = input.items.find(
        (candidate) => candidate.itemId === item.id,
      );
      if (
        !approval ||
        !validApproval(item.quantityRequested, approval.quantityApproved)
      )
        throw new AppError(
          422,
          "INVALID_QUANTITY",
          "Approved quantities must be between zero and the requested quantities.",
        );
      await tx
        .update(borrowRequestItems)
        .set({ quantityApproved: approval.quantityApproved })
        .where(eq8(borrowRequestItems.id, item.id));
    }
    const [updated] = await tx
      .update(borrowRequests)
      .set({
    ...
    await tx.insert(auditLogs).values({
      ...
      entityId: id,
      metadata: {
        quantities: input.items.map((item) => ({
          itemId: item.itemId,
...
async function borrowingRoutes(app2) {
  ...
  app2.post("/api/v1/borrow-requests/", async (request, reply) => {
    ...
    return reply
      .code(201)
      .send(
        await createDraft(borrowRequestInput.parse(request.body), actor.id),
      );
  });
  ...
  app2.patch("/api/v1/borrow-requests/:id/", async (request) => {
    ...
    return updateDraft(
      uuidParam.parse(request.params).id,
      borrowRequestInput.parse(request.body),
      actor.id,
    );
  });
  ...
  app2.post("/api/v1/borrow-requests/:id/approve/", async (request) => {
    ...
    return approveRequest(
      uuidParam.parse(request.params).id,
      actor.id,
      approvalInput.parse(request.body),
    );
  });
  app2.post("/api/v1/borrow-requests/:id/reject/", async (request) => {
    ...
    return rejectRequest(
      uuidParam.parse(request.params).id,
      actor.id,
      rejectionInput.parse(request.body),
    );
  });
  app2.post(
    "/api/v1/borrow-requests/:id/allocations/",
    async (request, reply) => {
      ...
      return reply
        ...
        .send(
          await allocateAsset(
            uuidParam.parse(request.params).id,
            actor.id,
            allocationInput.parse(request.body),
          ),
        );
    },
  );
  app2.post("/api/v1/borrow-requests/:id/release/", async (request) => {
    ...
    return releaseRequest(
      uuidParam.parse(request.params).id,
      actor.id,
      releaseInput.parse(request.body),
    );
  });
  app2.post("/api/v1/returns/", async (request, reply) => {
    ...
    return reply
      .code(201)
      .send(await processReturn(actor.id, returnInput.parse(request.body)));
  });
  app2.get("/api/v1/custody/", async (request) => {
    const actor = await requireActiveProfile(request);
    if (actor.role === "admin")
      throw new AppError(403, "FORBIDDEN", "Custody records are restricted.");
    const { page, limit } = listQuery.parse(request.query);
    return listCustody(
      page,
      limit,
      actor.role === "student_faculty" ? actor.id : void 0,
    );
  });
  ...
  app2.get("/api/v1/accountability/", async (request) => {
    ...
    if (actor.role === "admin")
      throw new AppError(
        403,
        "FORBIDDEN",
        "Accountability records are restricted.",
      );
    const { page, limit } = listQuery.parse(request.query);
...
async function createRequisition(requestId, actorId) {
  const record = await database().transaction(async (tx) => {
    const [request] = await tx
      .select()
      .from(borrowRequests)
      .where(eq9(borrowRequests.id, requestId))
      .for("update")
      .limit(1);
    if (!request) throw new AppError(404, "NOT_FOUND", "Request not found.");
    if (
    ...
    const [existing] = await tx
      ...
      .where(eq9(isoRequisitions.borrowRequestId, requestId))
      .limit(1);
...
async function generateRequisitionPdf(id, actorId) {
  const [record] = await database()
    .select()
    .from(isoRequisitions)
    .where(eq9(isoRequisitions.id, id))
    .limit(1);
...
async function listRequisitions(page, limit) {
  const [data, totals] = await Promise.all([
    database()
      ...
      .orderBy(desc2(isoRequisitions.generatedAt))
      .limit(limit)
      .offset((page - 1) * limit),
...
async function markRequisitionReleased(id, actorId) {
  return database().transaction(async (tx) => {
    ...
    const [updated] = await tx
      .update(isoRequisitions)
      .set({ releasedAt: /* @__PURE__ */ new Date().toISOString() })
      .where(eq9(isoRequisitions.id, id))
      .returning();
    await tx.insert(auditLogs).values({
      actorUserId: actorId,
...
async function notificationRoutes(app2) {
  app2.get("/api/v1/notifications/", async (request) => {
    ...
    const [data, totals, unread] = await Promise.all([
      database()
        .select()
        .from(notifications)
        .where(eq10(notifications.userId, actor.id))
        .orderBy(desc3(notifications.createdAt))
        .limit(limit)
        .offset((page - 1) * limit),
      database()
        .select({ total: count7() })
      ...
      database()
        ...
        .where(
          and7(
            eq10(notifications.userId, actor.id),
  ...
  app2.post("/api/v1/notifications/:id/read/", async (request) => {
    ...
    const [row] = await database()
      ...
      .set({ readAt: /* @__PURE__ */ new Date().toISOString() })
      .where(
        and7(eq10(notifications.id, id), eq10(notifications.userId, actor.id)),
      )
...
async function analyticsRoutes(app2) {
  app2.get("/api/v1/analytics/:key/", async (request) => {
    ...
    const [data, totals] = await Promise.all([
      database().execute(
        sql5`select * from ${view} limit ${limit} offset ${(page - 1) * limit}`,
      ),
  ...
  app2.get("/api/v1/reports/:key/", async (request) => {
    ...
    if (!reports[key].roles.includes(actor.role))
      throw new AppError(
        403,
        "FORBIDDEN",
    ...
    const [data, totals] = await Promise.all([
      database().execute(
        sql5`select * from ${source} limit ${limit} offset ${(page - 1) * limit}`,
      ),
  ...
  app2.get("/api/v1/reports/:key/csv/", async (request, reply) => {
    ...
    const rows = await database().execute(
      sql5`select * from ${sql5.raw(reports[key].source)} limit 10000`,
    );
...
async function dashboardRoutes(app2) {
  app2.get("/api/v1/dashboard/", async (request) => {
    ...
    if (actor.role === "super_admin") {
      const [requests2, custody2, overdue, returns, audit, activity] =
        await Promise.all([
          database().execute(
            sql6`select status::text as status, count(*)::int as total from borrow_requests group by status`,
          ),
          database().execute(
            sql6`select count(*)::int as total from vw_current_custody`,
          ),
          database().execute(
            sql6`select count(*)::int as total from vw_overdue_loans`,
          ),
          database().execute(
            sql6`select count(*)::int as total from return_records where returned_at > now() - interval '7 days'`,
          ),
          database().execute(
            sql6`select count(*)::int as total, count(*) filter (where u.role = 'admin')::int as admin_total from audit_logs a left join users u on u.id = a.actor_user_id`,
          ),
          database().execute(
            sql6`select a.id, a.action, a.entity_type as "entityType", a.created_at as "createdAt", u.first_name as "firstName", u.last_name as "lastName", u.role as "actorRole" from audit_logs a left join users u on u.id = a.actor_user_id order by a.created_at desc, a.id desc limit 8`,
          ),
        ]);
      const countStatus2 = (status) =>
        Number(requests2.find((row) => row.status === status)?.total ?? 0);
      return {
        audit: {
        ...
        cards: [
          ...
          { label: "Under review", value: countStatus2("under_review") },
          {
            ...
            value: countStatus2("ready_for_release"),
          },
    ...
    if (actor.role === "admin") {
      const [assets, pending, usage] = await Promise.all([
        database().execute(
          sql6`select availability_status, count(*)::int as total from vw_equipment_asset_availability group by availability_status`,
        ),
        database().execute(
          sql6`select count(*)::int as total from users where account_status = 'pending'`,
        ),
        database().execute(
          sql6`select coalesce(sum(total_borrow_count), 0)::int as total from vw_equipment_usage`,
        ),
      ]);
    ...
    const [requests, custody, notifications2] = await Promise.all([
      database().execute(
        sql6`select status::text as status, count(*)::int as total from borrow_requests where borrower_id = ${actor.id}::uuid group by status`,
      ),
      database().execute(
        sql6`select count(*)::int as total from vw_current_custody where borrower_id = ${actor.id}::uuid`,
      ),
      database().execute(
        sql6`select count(*)::int as total from notifications where user_id = ${actor.id}::uuid and read_at is null`,
      ),
    ]);
    ...
    return {
      cards: [
        ...
        { label: "Assets in custody", value: Number(custody[0].total) },
        {
          ...
          value: Number(notifications2[0].total),
        },
      ],
    };
  });
...
function auditScope(viewerRole, requestedRole) {
  if (viewerRole === "admin")
    return {
      ...
      deny: Boolean(requestedRole && requestedRole !== "student_faculty"),
    };
...
async function auditRoutes(app2) {
  app2.get("/api/v1/audit/", async (request) => {
    ...
    const filter = and8(
      ...
      input.q
        ? or6(
            ilike5(auditLogs.action, `%${input.q}%`),
            ilike5(users.firstName, `%${input.q}%`),
            ilike5(users.lastName, `%${input.q}%`),
            ilike5(users.institutionalId, `%${input.q}%`),
            sql7`${auditLogs.metadata}::text ilike ${`%${input.q}%`}`,
            sql7`${auditLogs.entityId}::text ilike ${`%${input.q}%`}`,
          )
        : void 0,
    );
    const [rows, totals] = await Promise.all([
      database()
        ...
        .from(auditLogs)
        .leftJoin(users, eq11(auditLogs.actorUserId, users.id))
        .where(filter)
        .orderBy(desc4(auditLogs.createdAt), desc4(auditLogs.id))
        .limit(input.limit)
        .offset((input.page - 1) * input.limit),
      database()
        .select({ total: count8() })
        ...
        if (ids.length)
          addNames(
            ...
            await database()
              ...
              .from(colleges)
              .where(inArray3(colleges.id, ids)),
          );
      })(),
      (async () => {
        ...
        if (ids.length)
          addNames(
            ...
            await database()
              ...
              .from(departments)
              .where(inArray3(departments.id, ids)),
          );
      })(),
      (async () => {
        ...
        if (ids.length)
          addNames(
            ...
            await database()
              ...
              .from(equipmentCategories)
              .where(inArray3(equipmentCategories.id, ids)),
          );
      })(),
      (async () => {
        ...
        if (ids.length)
          addNames(
            ...
            await database()
              ...
              .from(equipmentCatalog)
              .where(inArray3(equipmentCatalog.id, ids)),
          );
      })(),
      (async () => {
          ...
          addNames(
            ...
            (
              await database()
                ...
                .innerJoin(
                  equipmentCatalog,
                  eq11(equipmentAssets.equipmentCatalogId, equipmentCatalog.id),
                )
                .where(inArray3(equipmentAssets.id, ids))
            ).map((row) => ({
              id: row.id,
              label: `${row.equipment} \xB7 ${row.code}`,
            })),
          );
      })(),
      (async () => {
          ...
          addNames(
            ...
            (
              await database()
                ...
                .from(borrowRequests)
                .innerJoin(users, eq11(borrowRequests.borrowerId, users.id))
                .where(inArray3(borrowRequests.id, ids))
            ).map((row) => ({
              id: row.id,
              label: `${row.number} \xB7 ${row.borrowerFirst} ${row.borrowerLast}`,
            })),
          );
      })(),
      (async () => {
          ...
          addNames(
            ...
            (
              await database()
                ...
                .innerJoin(
                  equipmentAssets,
                  eq11(borrowAllocations.equipmentAssetId, equipmentAssets.id),
                )
                .innerJoin(
                  borrowRequestItems,
                  eq11(
                ...
                .innerJoin(
                  borrowRequests,
                  eq11(borrowRequestItems.borrowRequestId, borrowRequests.id),
                )
                .where(inArray3(borrowAllocations.id, ids))
            ).map((row) => ({
              id: row.id,
          ...
          addNames(
            ...
            (
              await database()
                ...
                .innerJoin(
                  borrowAllocations,
                  eq11(returnRecords.allocationId, borrowAllocations.id),
                )
                .innerJoin(
                  equipmentAssets,
                  eq11(borrowAllocations.equipmentAssetId, equipmentAssets.id),
                ...
                .where(inArray3(returnRecords.id, ids))
            ).map((row) => ({
              id: row.id,
              label: `${row.code} for request ${row.number}`,
            })),
        ...
        if (ids.length)
          addNames(
            ...
            await database()
              ...
              .from(isoRequisitions)
              .where(inArray3(isoRequisitions.id, ids)),
          );
      })(),
    ]);
    return {
      ...
      if (passwordChange && response.ok && passwordSession) {
        try {
          const [actor] = await database()
            .select({ id: users.id })
            .from(users)
            .where(eq12(users.authUserId, passwordSession.user.id))
            .limit(1);
          if (actor)
            await database()
              .insert(auditLogs)
