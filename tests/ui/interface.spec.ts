...
const routes = {
  super_admin: ["/super-admin/", "/super-admin/users/", "/super-admin/requests/", `/super-admin/requests/${id}/`, `/super-admin/requests/${id.slice(0, -1)}2/`, `/super-admin/requests/${id.slice(0, -1)}3/`, `/super-admin/requests/${id.slice(0, -1)}4/`, "/super-admin/custody/", "/super-admin/overdue/", "/super-admin/accountability/", "/super-admin/iso/", "/super-admin/analytics/", "/super-admin/reports/", "/super-admin/audit/", "/super-admin/organization/colleges/", "/super-admin/organization/courses/", "/super-admin/organization/departments/", "/super-admin/staff/new/", "/account/profile/", "/notifications/", "/account/password/", `/equipment/q/${id}/`],
  admin: ["/admin/", "/admin/users/", "/admin/equipment/", "/admin/equipment/categories/", "/admin/equipment/catalog/", "/admin/equipment/assets/", "/admin/analytics/", "/admin/reports/", "/admin/audit/", "/account/profile/"],
...
test("missing URLs show a clear recovery page", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /Open your dashboard/ })).toBeVisible();
  await expect(page.getByRole("link", { name: /Back to home/ })).toBeVisible();
});

test("collapsed add panels stand out from surrounding cards", async ({ page }) => {
  ...
  expect(await summary.evaluate((element) => getComputedStyle(element).backgroundColor)).toBe("rgb(43, 80, 54)");
  await summary.click();
...
test("Admin audit shows student and faculty actions only", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Student activity" })).toHaveAttribute("aria-current", "page");
  ...
  await expect(page.getByRole("combobox", { name: "Actor role" })).toHaveCount(0);
});

test("registration uses a custom options menu and shared button styles", async ({ page }) => {
  ...
  expect(await landingButton.evaluate((element) => getComputedStyle(element).borderRadius)).toBe("12px");
  await landingButton.hover();
  expect(await landingButton.evaluate((element) => getComputedStyle(element).transform)).toBe("none");
});
...
test("landing preview and auth pages support scrolling", async ({ page }) => {
  ...
  await expect(page.getByText("Illustrative preview.", { exact: false })).toBeVisible();
...
test("landing switches navigation when a session exists", async ({ page }) => {
  ...
  await role(page, "admin");
  await page.reload();
  await expect(page.getByRole("link", { name: "Open dashboard" })).toHaveAttribute("href", "/admin/");
  await expect(page.getByRole("button", { name: "Sign out" })).toBeVisible();
  await expect(page.getByRole("link", { name: "Sign in" })).toHaveCount(0);
  await expect(page.getByRole("link", { name: "Create account" })).toHaveCount(0);
});

test("sidebar keeps its scroll position across dashboard navigation", async ({ page }) => {
  await role(page, "super_admin");
  await page.setViewportSize({ width: 1280, height: 600 });
  await page.goto("/super-admin/");
  const navigation = page.getByRole("navigation", { name: "Main navigation" });
  await navigation.evaluate((element) => { element.scrollTop = 260; });
  ...
  expect(await navigation.evaluate((element) => element.scrollTop)).toBeGreaterThanOrEqual(before - 2);
});

test("Admin cannot suspend or archive accounts in the interface", async ({ page }) => {
  await role(page, "admin");
  await page.goto("/admin/users/");
  await expect(page.getByRole("button", { name: "Activate", exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: /Suspend|Archive/ })).toHaveCount(0);
  await page.getByRole("combobox", { name: "Account status" }).click();
  await page.getByRole("option", { name: "Active" }).click();
  await expect(page.getByRole("button", { name: /Suspend|Archive/ })).toHaveCount(0);
});

test("request date picker uses a custom accessible calendar", async ({ page }) => {
  await role(page, "student_faculty");
  await page.goto("/portal/requests/new/");
  await page.getByRole("button", { name: "Choose borrow date and time" }).click();
  const calendar = page.getByRole("dialog", { name: "Borrow date and time calendar" });
  ...
  await expect(page.getByLabel("Borrow date and time", { exact: true })).not.toHaveValue("");
...
test("super admin manages all account roles and keeps own access protected", async ({ page }) => {
  await role(page, "super_admin");
  await page.goto("/super-admin/users/");
  await expect(page.getByRole("row").filter({ hasText: "TEST-001" })).toBeVisible();
  await expect(page.getByRole("row").filter({ hasText: "ADMIN-001" })).toBeVisible();
  ...
  await page.getByRole("dialog").getByRole("button", { name: "Keep working" }).click();
  ...
  const update = page.waitForRequest((request) => request.method() === "PATCH" && request.url().includes("/api/v1/users/"));
  await editor.getByRole("button", { name: "Save details" }).click();
  const body = (await update).postDataJSON();
  expect(body).toMatchObject({ firstName: "Updated", email: "admin@example.invalid" });
  expect(body).not.toHaveProperty("role");
  expect(body).not.toHaveProperty("personType");
});

test("student profile edits only name and email; asset status uses custom options", async ({ page }) => {
  await role(page, "student_faculty");
  await page.goto("/portal/profile/");
  await expect(page.locator("aside").getByRole("link", { name: /Test Borrower/ })).toContainText("student");
  await expect(page.getByText("Student", { exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Edit name and email" }).click();
  await page.getByLabel("First name").fill("Updated");
  const update = page.waitForRequest((request) => request.method() === "PATCH" && request.url().includes("/api/v1/me/"));
  await page.getByRole("button", { name: "Save changes" }).click();
  const body = (await update).postDataJSON();
  expect(body).toMatchObject({ firstName: "Updated", email: "test.account@example.invalid" });
  expect(body).not.toHaveProperty("personType");
  expect(body).not.toHaveProperty("role");
  ...
  await expect(page.getByRole("combobox", { name: "Operational status" })).toContainText("Maintenance");
});
...
test("return requires inspected condition and damage remarks", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("combobox", { name: "Return outcome" })).toContainText("Damaged");
  await expect(page.getByRole("button", { name: "Confirm return" })).toBeDisabled();
...
test("offline feedback and retrying submission preserve the saved draft", async ({ page }) => {
  ...
  await page.getByLabel("Borrow date and time", { exact: true }).fill("2026-10-01T09:00");
  await page.getByLabel("Due date and time", { exact: true }).fill("2026-10-02T09:00");
...
test("dashboard filter links, review validation, and release confirmation", async ({ page }) => {
  ...
  await expect(page.getByRole("combobox", { name: "Request status" })).toContainText("Submitted");
...
test("dropdowns are opaque and empty panels explain what happens next", async ({ page }) => {
  await role(page, "admin");
  await page.goto("/admin/reports/");
  await page.getByRole("combobox", { name: "Report" }).click();
  const menu = page.locator(".custom-select-content");
  await expect(menu).toBeVisible();
  expect(await menu.evaluate((element) => getComputedStyle(element).backgroundColor)).toBe("rgb(255, 255, 255)");
  await expect(page.getByRole("option", { name: "Inventory" })).toBeVisible();
  await page.keyboard.press("Escape");
  await page.goto("/notifications/");
  await expect(page.getByText("Your updates")).toBeVisible();
  await expect(page.getByRole("list").filter({ hasText: "Request approved" })).toBeVisible();
});

test("empty notifications explain where future updates will appear", async ({ page }) => {
  await role(page, "admin");
  await page.route("**/api/v1/notifications/**", (route) => route.fulfill({ json: { data: [], total: 0, unreadCount: 0 } }));
  await page.goto("/notifications/");
  await expect(page.getByRole("heading", { name: "No notifications yet" })).toBeVisible();
  await expect(page.getByText("Updates about your account and laboratory activity will appear here.")).toBeVisible();
  await expect(page.getByRole("button", { name: "Previous" })).toHaveCount(0);
});

test("audit activity shows readable actions and record names without UUIDs", async ({ page }) => {
  await role(page, "super_admin");
  ...
  await expect(table.getByText("Physical asset: Compound microscope · MIC-003")).toBeVisible();
  await expect(table).not.toContainText(id);
});

test("empty equipment and department menus explain the missing setup", async ({ page }) => {
  await role(page, "admin");
  await page.route("**/api/v1/equipment/catalog/**", (route) => route.fulfill({ json: { data: [], total: 0 } }));
  await page.goto("/admin/equipment/assets/");
  await page.locator("summary").filter({ hasText: "Add physical asset" }).click();
  await page.getByRole("combobox", { name: "Equipment type" }).click();
  await expect(page.getByText("No equipment types yet. Add an equipment category and type before recording an asset.")).toBeVisible();
  await page.keyboard.press("Escape");
  await role(page, "super_admin");
  await page.route("**/api/v1/departments/**", (route) => route.fulfill({ json: { data: [], total: 0 } }));
  await page.goto("/super-admin/staff/new/");
  await page.getByRole("combobox", { name: "Department (optional)" }).click();
  await expect(page.getByText("No departments have been added yet. You can create this Admin without assigning a department.")).toBeVisible();
});

test("registration explains when a chosen college has no courses", async ({ page }) => {
  await page.route("**/api/v1/courses/**", (route) => route.fulfill({ json: { data: [], total: 0 } }));
  await page.goto("/register/");
  ...
  await expect(page.getByText("No courses are available for this college. Choose another college or contact the laboratory administrator.")).toBeVisible();
});

test("admin and super admin see graphs for every analysis", async ({ page }) => {
  const analyses = ["Monthly borrowing trends", "Equipment usage", "Inventory utilization", "Borrowing frequency by user", "Borrowing frequency by college", "Borrowing frequency by course", "Peak borrowing periods", "Inventory overview"];
  for (const accountRole of ["admin", "super_admin"]) {
    await role(page, accountRole);
    await page.goto(`/${accountRole === "admin" ? "admin" : "super-admin"}/analytics/`);
    ...
    for (const title of analyses) {
      ...
      await expect(page.getByRole("img", { name: `${title} graph. Exact values are listed in the table below.` })).toBeVisible();
      await expect(page.getByRole("region", { name: "Analytics data" })).toBeVisible();
    }
  }
});
