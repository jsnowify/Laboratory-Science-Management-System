...
const labels: Record<string, [string, Tone]> = {
  ...
  ready_for_release: ["Ready for release", "success"],
  borrowed: ["Borrowed", "info"],
  partially_returned: ["Partially returned", "warning"],
  ...
  maintenance_required: ["Maintenance required", "warning"],
};
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border px-2.5 py-1 text-xs font-semibold ${styles[tone]}`}
    >
