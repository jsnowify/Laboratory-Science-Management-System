...
try {
  ...
  for (const row of triggers)
    console.log(
      `${row.event_object_table}: ${row.trigger_name} ${row.action_timing} ${row.event_manipulation} ${row.action_statement}`,
    );
  ...
  for (const row of functions)
    console.log(`\n${row.proname}:\n${row.definition}`);
  const triggerDefs =
    await db`select tgname, pg_get_triggerdef(t.oid) as definition from pg_trigger t where t.tgrelid='borrow_allocations'::regclass and not t.tgisinternal order by tgname`;
  for (const row of triggerDefs)
    console.log(`TRIGGER ${row.tgname}: ${row.definition}`);
} finally {
  await db.end();
}
