...
export function PortalCatalog() {
  ...
  return (
    <div className="space-y-5">
      ...
      >
        <div className="relative min-w-[190px] flex-1 sm:max-w-md"><Search size={18} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#52725b]" aria-hidden="true" />
        <input
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><CardLoadingSkeleton label="Loading equipment" /><CardLoadingSkeleton label="Loading equipment" /><CardLoadingSkeleton label="Loading equipment" /></div>
      ) : items.length === 0 ? (
        <p className="rounded-xl border border-slate-200 bg-white p-8 text-center text-slate-600">
          {search ? "No equipment matches your search. Try a different keyword." : "No equipment is available to browse yet. Check back after the laboratory adds equipment."}
        </p>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {items.map((item) => (
            ...
            >
              <div className="flex items-start justify-between gap-3"><span className="flex size-12 items-center justify-center rounded-2xl bg-[#e9f3e9] text-[#366e4a]"><Boxes size={23} aria-hidden="true" /></span><span className="rounded-full border border-[#d8e8d8] bg-[#f3f8f2] px-3 py-1 text-xs font-semibold text-[#2b6743]">{item.availableUnits} available</span></div>
              <h2 className="mt-6 text-xl font-semibold text-[#213f2e]">{item.equipmentName}</h2>
              {item.description && (
              ...
              <dl className="mt-5 grid grid-cols-2 gap-3 rounded-xl bg-[#f6f9f5] p-4 text-sm">
                ...
                <div>
                  ...
                  <dd className="mt-1 text-lg font-semibold text-[#284534]">{item.totalUnits}</dd>
                </div>
              </dl>
              <Link
              ...
              >
                Request this equipment <ArrowUpRight size={18} aria-hidden="true" />
              </Link>
            </article>
          ))}
        </div>
