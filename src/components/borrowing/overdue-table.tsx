...
export function OverdueTable() {
  ...
  useEffect(() => {
    ...
    apiRequest<{ data: Overdue[]; total: number }>("/api/v1/overdue/", {
      query: { page, limit: 20 },
        ...
        if (!controller.signal.aborted)
          setError(
            cause instanceof Error
              ? cause.message
              : "Overdue records could not be loaded.",
          );
      })
      .finally(() => {
              ...
              <tr>
                <td colSpan={6} className="p-8 text-center text-slate-500">
                  {error
                    ...
                    : "No overdue equipment. Items will appear here if they pass their due date."}
                </td>
              </tr>
            ) : (
              rows.map((row) => (
