...
import { profileUpdateInput, userListQuery, userStatusInput, type Role } from "@lsms/shared";
import { database } from "../../db";
import { authUser } from "../../db/auth-schema";
import { auditLogs, notifications, users } from "../../db/introspected/schema";
import { AppError } from "../../lib/errors";
import { canAdminChangeStatus, canSetStatus, canSuperAdminChangeStatus } from "./account-rules";
...
export async function listManageableUsers(
  ...
  const filter = and(
    actorRole === "admin" ? eq(users.role, "student_faculty") : input.role ? eq(users.role, input.role) : undefined,
...
export async function changeUserStatus(
  ...
  return database().transaction(async (tx) => {
    if (actorRole === "super_admin") await tx.execute(sql`select pg_advisory_xact_lock(742611931)`);
    ...
    if (actorRole === "super_admin" && target.role === "super_admin" && target.accountStatus === "active" && input.status !== "active") {
      const [other] = await tx.select({ id: users.id }).from(users).where(and(eq(users.role, "super_admin"), eq(users.accountStatus, "active"), ne(users.id, id))).limit(1);
...
export async function updateAccountDetails(id: string, input: z.infer<typeof profileUpdateInput>, actorId: string, actorRole: Role) {
  ...
  return database().transaction(async (tx) => {
    ...
    const [duplicate] = await tx.select({ id: users.id }).from(users).where(eq(users.email, input.email)).limit(1);
    ...
    const [authRow] = await tx.update(authUser).set({ name: fullName, email: input.email, emailVerified: target.email.toLowerCase() === input.email ? undefined : false, updatedAt: new Date() }).where(eq(authUser.id, target.authUserId)).returning({ id: authUser.id });
    if (!authRow) throw new AppError(409, "AUTH_MISSING", "This account has no sign-in record.");
    const [row] = await tx.update(users).set({ firstName: input.firstName, middleName: input.middleName || null, lastName: input.lastName, email: input.email, updatedAt: new Date().toISOString() }).where(eq(users.id, id)).returning({ id: users.id, firstName: users.firstName, middleName: users.middleName, lastName: users.lastName, email: users.email });
    await tx.insert(auditLogs).values({ actorUserId: actorId, action: actorId === id ? "profile.updated" : "user.profile_updated", entityType: "users", entityId: id, metadata: { targetInstitutionalId: target.institutionalId, changedFields: ["firstName", "middleName", "lastName", "email"].filter((field) => String(target[field as keyof typeof target] ?? "") !== String(input[field as keyof typeof input] ?? "")) } });
    return row;
  });
}
