...
const name = z
  ...
  .max(100)
  .refine(
    (value) => !unsafeInline.test(value),
    "Remove control or hidden direction characters.",
  );
const email = z
  .string()
  .trim()
...
export const staffInput = identityInput.extend({
  departmentId: z.uuid().optional(),
});
export const registrationInput = identityInput
  .extend({
...
export const departmentInput = organizationInput.extend({
...
export const organizationUpdate = organizationInput
  .partial()
...
export const courseUpdate = courseInput
  .partial()
  .extend({ isActive: z.boolean().optional() })
  .refine(
    (value) => Object.keys(value).length > 0,
...
export const departmentUpdate = departmentInput
  .partial()
  .extend({ isActive: z.boolean().optional() })
  .refine(
    (value) => Object.keys(value).length > 0,
...
export const categoryUpdate = categoryInput
  .partial()
  .extend({ isActive: z.boolean().optional() })
...
export const catalogUpdate = catalogInput
  .partial()
  .extend({ isActive: z.boolean().optional() })
  .refine((value) => Object.keys(value).length > 0);
...
export const borrowRequestInput = z
  .object({
...
export const borrowListQuery = listQuery.extend({
  status: z
    .enum([
      "draft",
      "submitted",
