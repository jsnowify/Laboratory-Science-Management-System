...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const profile = loginProfile.parse(
        await apiRequest<Profile>("/api/v1/me/"),
      );
      const destination =
  ...
  return (
    <AuthFrame title="Sign in" subtitle="Use your LSMS account.">
      <form onSubmit={submit} className="space-y-5" aria-busy={busy}>
        <FormField id="email" label="Institutional email">
          <input
            id="email"
            type="email"
            autoComplete="email"
      ...
      <div className="mt-8 border-t border-[#e5ece3] pt-6">
        <p className="text-center text-sm text-[#647367]">
        ...
        <p className="mt-5 flex items-start gap-2 rounded-xl bg-[#f2f6f0] p-3 text-xs leading-5 text-[#415c48]">
          <LockKeyhole
            size={16}
            className="mt-0.5 shrink-0 text-[#38734b]"
            aria-hidden="true"
          />{" "}
          Your account must be activated by a laboratory administrator before
