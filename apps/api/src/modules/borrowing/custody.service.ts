...
export async function allocateAsset(
  ...
  return database().transaction(async (tx) => {
    ...
    const [item] = await tx
      ...
      .where(eq(borrowRequestItems.id, input.itemId))
      .for("update")
      .limit(1);
    ...
    const [asset] = await tx
      ...
      .where(eq(equipmentAssets.id, input.assetId))
      .limit(1);
    if (
      !asset ||
    ...
    const [overlap] = await tx
      ...
      .where(
        and(
          eq(borrowAllocations.equipmentAssetId, input.assetId),
          inArray(borrowAllocations.allocationStatus, ["reserved", "released"]),
          sql`${borrowAllocations.reservationPeriod} && tstzrange(${request.requestedBorrowAt}::timestamptz, ${request.requestedDueAt}::timestamptz, '[)')`,
        ),
      )
      .limit(1);
    if (overlap)
    ...
    const [allocated] = await tx
      ...
      .where(
        and(
          eq(borrowAllocations.borrowRequestItemId, item.id),
          inArray(borrowAllocations.allocationStatus, ["reserved", "released"]),
        ),
      );
    ...
    const [total] = await tx
      ...
      .innerJoin(
        borrowRequestItems,
        eq(borrowAllocations.borrowRequestItemId, borrowRequestItems.id),
      )
      .where(
        and(
          eq(borrowRequestItems.borrowRequestId, requestId),
          inArray(borrowAllocations.allocationStatus, ["reserved", "released"]),
        ),
      );
    if (
  ...
  return database().transaction(async (tx) => {
    ...
    for (const item of input.allocations) {
      await tx
        ...
        .where(eq(borrowAllocations.id, item.allocationId));
    }
...
export async function processReturn(
  ...
  return database().transaction(async (tx) => {
    const [allocation] = await tx
      ...
      .where(eq(borrowAllocations.id, input.allocationId))
      .for("update")
      .limit(1);
    if (!allocation)
    ...
    const [request] = await tx
      .select()
      .from(borrowRequests)
      .where(eq(borrowRequests.id, allocation.borrowRequestId))
      .for("update")
      .limit(1);
    if (
      !request ||
    ...
    const [totals] = await tx
      .select({
        total: count(),
        returned: sql<number>`count(*) filter (where ${borrowAllocations.allocationStatus} = 'returned')::int`,
      })
      .from(borrowAllocations)
      .innerJoin(
        borrowRequestItems,
      ...
      .where(
        and(
          eq(borrowRequestItems.borrowRequestId, request.id),
    ...
    await tx
      .update(borrowRequests)
      .set({ status })
      .where(eq(borrowRequests.id, request.id));
    await tx
      .insert(auditLogs)
      .values({
        actorUserId: actorId,
...
export async function listCustody(
  ...
  const filter = borrowerId
    ? eq(vwCurrentCustody.borrowerId, borrowerId)
    : undefined;
...
export async function listAccountability(
  page: number,
  limit: number,
  borrowerId?: string,
) {
  const filter = borrowerId
    ? eq(vwAccountabilityRecords.borrowerId, borrowerId)
    : undefined;
  const [data, totals] = await Promise.all([
    database()
      .select()
